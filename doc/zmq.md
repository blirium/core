...
The Blirium Core daemon can be configured to act as a trusted "border
router", implementing the blirium wire protocol and relay, making
...
The ZeroMQ feature in Blirium Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bliriumd:
...
For instance:
    ...
    $ bliriumd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/bliriumd.tx.raw \
...
These options can also be provided in blirium.conf.
...
From the perspective of bliriumd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bliriumd directly. Furthermore, no information is
...
using. Bliriumd appends an up-counting sequence number to each
