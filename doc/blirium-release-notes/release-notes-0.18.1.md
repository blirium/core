Blirium Core version 0.18.1 is now available from:

  <https://download.blirium.org/blirium-0.18.1/>
...
`/Applications/Blirium-Qt` (on Mac) or `bliriumd`/`blirium-qt` (on
Linux).
...
Blirium Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Blirium Core on unsupported systems.
...
10.10. Additionally, Blirium Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:17627:17627` (this is an extra `:17627` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@blirium.org) the Blirium Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 BLR),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in BLR per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/blirium-conf.md)
  about the `blirium.conf` file describes how to use it to configure
  Blirium Core.

- A new document introduces Blirium Core's BIP174 [Partially-Signed
  Blirium Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Blirium-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Blirium Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Blirium Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Blirium Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart bliriumd with the `-deprecatedrpc=generate`
...
- A new `blirium-wallet` tool is now distributed alongside Blirium
...
This section describes planned changes to Blirium Core that may affect
other Blirium software and services.

- Since version 0.16.0, Blirium Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Blirium Core 0.20 (expected about
  a year after 0.18), Blirium Core will default to native segwit
  ...
  to bech32 addresses, and if the Blirium Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Blirium Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Blirium Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Blirium Core
...
- A sub-project of Blirium Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Blirium Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Blirium Core's own implementation, although entropy
  gathered by Blirium Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Blirium Core a
...
- On macOS, Blirium Core now opts out of application CPU throttling
...
- #17627 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
